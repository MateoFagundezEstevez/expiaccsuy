...
with col2:
    logo = Image.open("logo_ccsuy.png")  # Asegúrate de tener el logo en el mismo directorio que tu archivo .py
...
# Subir archivos
mercados_file = st.file_uploader("Cargar mercados.csv", type="csv")
afinidad_file = st.file_uploader("Cargar afinidad_producto_país.csv", type="csv")
acuerdos_file = st.file_uploader("Cargar acuerdos_comerciales.csv", type="csv")

# Verificar si los archivos se cargaron correctamente
if mercados_file is not None and afinidad_file is not None and acuerdos_file is not None:
    # Cargar los datos
    mercados_df = pd.read_csv(mercados_file, encoding='latin1')
    afinidad_df = pd.read_csv(afinidad_file)
    acuerdos_df = pd.read_csv(acuerdos_file, encoding='latin1')
    ...
    st.warning("Por favor, carga los archivos CSV necesarios para continuar.")
